  ...
  const guides: Record<ProviderName, GuideInfo> = {
    zoom: {
      ...
      steps: [
        ...
        'Click "Build App" → choose <strong>OAuth</strong>',
        'Set the Redirect URL to: <code style="background:var(--bg);padding:1px 4px;border-radius:3px;font-size:12px;">recap://oauth/zoom/callback</code>',
        "Add scopes: <strong>meeting:read</strong>, <strong>recording:read</strong>, <strong>user:read</strong>",
        "Copy the <strong>Client ID</strong> and <strong>Client Secret</strong> from the App Credentials page",
      ],
