...
- **Org slug vs. subfolder:** `event.org` is the slug for frontmatter identity (`org:`); on-disk path comes from `OrgConfig.resolve_subfolder(vault_path)` in `sync.py`/`scheduler.py` (detector still hand-joins `org_config.subfolder` to its own vault root — Phase 3 cleanup target). The legacy `sync.org_subfolder()` hardcode is gone
