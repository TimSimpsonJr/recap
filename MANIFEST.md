...
- **Speaker correction + identity model (#28):** `Utterance.speaker_id` is immutable (diarized identity, written once); `Utterance.speaker` is the mutable display label rewritten by `_apply_speaker_mapping` keyed on `speaker_id`. Clip lookup + cache filename key on `speaker_id` so rewrites don't break playback. `KnownContact` gains `aliases: list[str]` + `email: str | None`; `match_known_contacts` matches email-first, then alias-aware normalized name. `resolve_recording_path(stem) -> Path | None` shared helper (FLAC->M4A precedence) used by both `/api/meetings/{stem}/speakers` (new) and the clip endpoint. Correction save (`POST /api/meetings/speakers`) atomically writes `.speakers.json`, applies contact mutations via ruamel round-trip, calls `daemon.refresh_config()` to propagate to subservices, creates People stubs via existing `_generate_person_stub` template, and triggers reprocess from `analyze` stage. Reprocess builds ephemeral `effective_participants` (re-canonicalized enrichment union correction-derived speakers, eligibility-filtered) and feeds both analyze + export. First-pass auto-relabel runs between diarize and analyze on Case A only (1 speaker_id + 1 eligible participant). Client-side resolution engine (8-step precedence: normalize -> email-first -> exact -> People-note-only -> strip-and-retry -> initial-aware near-match -> ineligibility -> create new) lives at `obsidian-recap/src/correction/resolve.ts` with Vitest coverage. Shared `_normalize` + `_is_eligible_person_label` in `recap/identity.py` mirrored by plugin `normalize.ts` for cross-language semantic parity. Endpoint error mapping: ValueError in contact mutations or stub creation -> 400 (client-correctable); unexpected Exception -> 500 with partial-success documented. Merge-friendly invariants preserved for #37: canonical `name` is single identity key; aliases never leak into note bodies or transcript labels.
